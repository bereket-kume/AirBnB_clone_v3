    ...
    def __init__(self, *args, **kwargs):
        ...
        if kwargs:
            ...
            if kwargs.get("created_at", None) and isinstance(self.created_at, str):
                self.created_at = datetime.strptime(kwargs["created_at"], time_format).replace(tzinfo=timezone.utc)
            else:
                self.created_at = datetime.now(timezone.utc)
            if kwargs.get("updated_at", None) and isinstance(self.updated_at, str):
                self.updated_at = datetime.strptime(kwargs["updated_at"], time_format).replace(tzinfo=timezone.utc)
            else:
